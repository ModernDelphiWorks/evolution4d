...
class function TObjectEx.AutoRefLock: TCriticalSection;
begin
  Result := FAutoRefLock;
end;

class function TObjectEx.Context: TRttiContext;
begin
  Result := FContext;
end;
...
class procedure TObjectEx.InitializeContext;
begin
  FContext := TRttiContext.Create;
  FAutoRefLock := TCriticalSection.Create;
end;

class procedure TObjectEx.FinalizeContext;
begin
  FAutoRefLock.Free;
  FContext.Free;
end;
...
function TObjectEx.Factory(const AClass: TClass; const AArgs: TArrayValue;
...
function TObjectEx.Factory<T>: T;
var
  LType: TRttiType;
  LInstance: TValue;
begin
...
begin
  ...
  begin
    TObjectEx.AutoRefLock.Acquire;
    try
      if not Assigned(FAutoRefLock) then
        FAutoRefLock := TAutoRefLock.Create;
    finally
      TObjectEx.AutoRefLock.Release;
    end;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        if FObjectEx = nil then
          FObjectEx := TObjectEx.New;
        LType := TObjectEx.Context.GetType(TypeInfo(T));
...
initialization
  TObjectEx.InitializeContext;

finalization
  TObjectEx.FinalizeContext;
