...
type
  ...
  public
    ...
    /// <summary>
    /// Simulates a list comprehension by transforming elements with an optional filter.
    /// </summary>
    /// <param name="ATransform">The function to transform each element.</param>
    /// <param name="APredicate">Optional predicate to filter elements (nil if no filter).</param>
    /// <returns>A new vector with the transformed elements, optionally filtered.</returns>
    function Comprehend<R>(const ATransform: TFunc<T, R>; const APredicate: TPredicate<T> = nil): TVector<R>;
  end;
...
function TListEx<T>.Comprehend<R>(const ATransform: TFunc<T, R>;
  const APredicate: TPredicate<T>): TVector<R>;
var
  LItem: T;
begin
  Result := TVector<R>.Create([]);
